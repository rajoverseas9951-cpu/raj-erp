...
class PolicyController
{
    public function index(Request $request): JsonResponse
    {
        ...
        $request->validate([
            ...
            'status' => ['sometimes', 'nullable', 'in:running,pending,expired,cancelled'],
        ...
        if ($request->filled('status')) $query->where('vehicle_insurances.status', $request->query('status'));
    ...
    public function summary(Request $request): JsonResponse
    {
        ...
        $commissions = DB::table('insurance_commissions')->where('tenant_id', $tenant)->whereNull('deleted_at');
        return response()->json(['success' => true, 'data' => [
            ...
            'tds' => round((float) (clone $commissions)->sum('tds_amount'), 2),
            'commission_received' => round((float) (clone $commissions)->sum('received_amount'), 2),
            'commission_outstanding' => round((float) (clone $commissions)->sum(DB::raw('net_receivable - received_amount')), 2),
        ]])->header('Cache-Control', 'private, no-store, no-cache, must-revalidate');
    }
