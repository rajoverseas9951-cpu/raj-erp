...
class VehicleOperationsController
{
    ...
    public function profile(Request $request, string $vehicle, VehicleModuleApplicabilityService $applicability)
    {
        ...
        foreach (self::TABLES as $module => $table) {
            if (! $this->isEnabled($rules, $module)) {
                continue;
            }
            if (in_array($module, self::FINANCIAL, true) && ! $this->financial($request)) {
                continue;
            }
        ...
        foreach ($groups as $name => $modules) {
            $groups[$name] = array_values(array_filter($modules, fn ($module) => ! in_array($module, self::FINANCIAL, true) || $this->financial($request)));
        }
    ...
    public function index(Request $request, string $vehicle, string $module)
    {
        ...
        if ($request->filled('search')) {
            $query->where(fn ($q) => $q->where('reference_number', 'like', '%'.$request->query('search').'%')->orWhere('notes', 'like', '%'.$request->query('search').'%'));
        }
        if ($request->filled('expires_within')) {
            $query->whereBetween('expiry_date', [now()->toDateString(), now()->addDays(min(30, max(1, (int) $request->query('expires_within'))))->toDateString()]);
        }
    ...
    public function store(Request $request, string $vehicle, string $module)
    {
        ...
        $table = $this->table($module);
        $this->guardApplicable($model, $module);
        $this->guardFinancial($request, $module, true);
        $data = $this->validated($request, $module);
        if (in_array($module, self::EXPIRY_MODULES, true)) {
            $data['status'] = VehicleModuleApplicabilityService::status($data['expiry_date'] ?? null);
        }
        $id = (string) Str::uuid();
        $now = now();
    ...
    public function update(Request $request, string $vehicle, string $module, string $record)
    {
        ...
        $query = DB::table($table)->where('tenant_id', $model->tenant_id)->where('vehicle_id', $model->id)->where('id', $record)->whereNull('deleted_at');
        abort_unless($query->exists(), 404);
        if (in_array($module, self::EXPIRY_MODULES, true)) {
            $data['status'] = VehicleModuleApplicabilityService::status($data['expiry_date'] ?? $query->value('expiry_date'));
        }
    ...
    public function destroy(Request $request, string $vehicle, string $module, string $record)
    {
        ...
        $this->guardApplicable($model, $module);
        $this->guardFinancial($request, $module, true);
        $updated = DB::table($table)->where('tenant_id', $model->tenant_id)->where('vehicle_id', $model->id)->where('id', $record)->whereNull('deleted_at')->update(['deleted_at' => now(), 'updated_by' => $request->user()?->id, 'updated_at' => now()]);
        abort_unless($updated, 404);
        $this->timeline($model, $request, 'vehicle.'.$module.'.deleted', Str::headline($module).' archived', $record);
    ...
    {
        $this->authorize($request, 'vehicle.documents');
        $model = $this->vehicle($request, $vehicle);
        $table = $this->table($module);
        ...
        $path = $file->store("tenants/{$model->tenant_id}/vehicles/{$model->id}/operations/{$module}", 'local');
        $id = (string) Str::uuid();
        ...
        $this->timeline($model, $request, 'vehicle.document.uploaded', 'Supporting document uploaded', $record);

        return response()->json(['success' => true, 'data' => DB::table('vehicle_operation_documents')->where('id', $id)->first()], 201);
    }
    ...
    public function downloadDocument(Request $request, string $vehicle, string $document)
    {
        $this->authorize($request, 'vehicle.documents');
        $model = $this->vehicle($request, $vehicle);
        $doc = DB::table('vehicle_operation_documents')->where('tenant_id', $model->tenant_id)->where('vehicle_id', $model->id)->where('id', $document)->whereNull('deleted_at')->first();
    ...
    public function deleteDocument(Request $request, string $vehicle, string $document)
    {
        ...
        DB::table('vehicle_operation_documents')->where('id', $doc->id)->update(['deleted_at' => now(), 'updated_at' => now()]);
        $this->timeline($model, $request, 'vehicle.document.deleted', 'Supporting document deleted', $doc->record_id);
    ...
    public function override(Request $request, string $vehicle)
    {
        ...
        $model = $this->vehicle($request, $vehicle);
        $data = $request->validate(['module' => ['required', Rule::in(array_keys(self::TABLES))], 'enabled' => ['required', 'boolean'], 'reason' => ['required', 'string', 'max:500']]);
        $key = ['tenant_id' => $model->tenant_id, 'vehicle_id' => $model->id, 'module' => $data['module']];
        $existing = DB::table('vehicle_module_overrides')->where($key)->first();
        DB::table('vehicle_module_overrides')->updateOrInsert($key, ['id' => $existing?->id ?? (string) Str::uuid(), 'enabled' => $data['enabled'], 'reason' => $data['reason'], 'created_by' => $request->user()?->id, 'created_at' => $existing?->created_at ?? now(), 'updated_at' => now(), 'deleted_at' => null]);
    ...
    {
        $this->authorize($request, 'vehicle.view');
        abort_unless(in_array($type, ['permit_type', 'rto_work_type', 'payment_type', 'other_payment_category'], true), 404);

        return response()->json(['success' => true, 'data' => DB::table('vehicle_operation_masters')->where('tenant_id', (string) $request->user()?->tenant_id)->where('type', $type)->where('is_active', true)->whereNull('deleted_at')->orderBy('name')->get()]);
    }
    ...
    public function storeMaster(Request $request, string $type)
    {
        $this->authorize($request, 'vehicle.update');
        abort_unless(in_array($type, ['permit_type', 'rto_work_type', 'payment_type', 'other_payment_category'], true), 404);
        $data = $request->validate(['name' => ['required', 'string', 'max:160'], 'code' => ['nullable', 'string', 'max:60']]);
        $tenant = (string) $request->user()?->tenant_id;
        $existing = DB::table('vehicle_operation_masters')->where('tenant_id', $tenant)->where('type', $type)->whereRaw('LOWER(name) = ?', [strtolower(trim($data['name']))])->whereNull('deleted_at')->first();
        if ($existing) {
            return response()->json(['success' => true, 'data' => $existing]);
        }
        $id = (string) Str::uuid();
        DB::table('vehicle_operation_masters')->insert(['id' => $id, 'tenant_id' => $tenant, 'type' => $type, 'name' => trim($data['name']), 'code' => $data['code'] ?? null, 'is_active' => true, 'created_at' => now(), 'updated_at' => now()]);

        return response()->json(['success' => true, 'data' => DB::table('vehicle_operation_masters')->where('id', $id)->first()], 201);
    }
    ...
    private function validated(Request $request, string $module, bool $partial = false): array
    {
        ...
        $columns = DB::getSchemaBuilder()->getColumnListing($this->table($module));

        return $request->validate(array_intersect_key($rules, array_flip($columns)));
    }

    private function present(object $row): array
    {
        $data = (array) $row;
        $data['derived_status'] = VehicleModuleApplicabilityService::status($row->expiry_date ?? null, true);
    ...
    {
        abort_unless(isset(self::TABLES[$module]), 404);
    ...
    {
        return Vehicle::where('tenant_id', (string) $request->user()?->tenant_id)->findOrFail($id);
    }

    private function financial(Request $request): bool
    {
        return (bool) ($request->user()?->is_admin || $request->user()?->can('vehicle.financial.view'));
    }

    private function guardFinancial(Request $request, string $module, bool $edit = false): void
    {
        if (in_array($module, self::FINANCIAL, true)) {
            abort_unless($request->user()?->is_admin || $request->user()?->can($edit ? 'vehicle.financial.edit' : 'vehicle.financial.view'), 403);
        }
    }
    ...
    private function isEnabled(array $rules, string $module): bool
    {
        return collect($rules['groups'])->flatten()->contains($module);
    }
    ...
    private function timeline(Vehicle $vehicle, Request $request, string $type, string $title, string $record): void
    {
        DB::table('vehicle_timeline_events')->insert(['id' => (string) Str::uuid(), 'tenant_id' => $vehicle->tenant_id, 'vehicle_id' => $vehicle->id, 'actor_id' => $request->user()?->id, 'event_type' => $type, 'title' => $title, 'description' => null, 'metadata' => json_encode(['record_id' => $record]), 'created_at' => now(), 'updated_at' => now()]);
    }
}
