...
const accountActions = [
  ...
  ["Party Balance", "Receivable / payable", "/accounts/outstanding", "wallet"],
...
export default function DashboardPage() {
  ...
  const refreshCompanyPayments = useCallback(async () => {
    try {
      ...
      const checks = await Promise.all(active.map(async (p) => {
        try {
          const s = await authenticatedRequest<SettlementInfo>(`/vehicles/${p.vehicle_id}/insurances/${p.id}/settlement`);
          return s.settlement ? null : p;
        } catch {
          return p;
        }
      }));
  ...
  const refresh = useCallback(() => {
    ...
    setRefreshing(true);
    setError("");
    void refreshCompanyPayments();
    void refreshBalances();
    return getDashboardSummary({ period, dateFrom, dateTo }, c.signal)
      .then(setData)
      .catch((e) => {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setClock(indiaNowParts(new Date())), 1000);
  ...
  useEffect(() => {
    ...
    window.addEventListener(DASHBOARD_REFRESH_EVENT, r);
    return () => {
      req.current?.abort();
      window.removeEventListener("focus", r);
      window.removeEventListener(DASHBOARD_REFRESH_EVENT, r);
    };
  }, [refresh]);
  ...
  const receivable = Number(balances?.summary.party_receivable ?? data?.kpis.outstanding_amount?.value ?? data?.revenue.outstanding ?? 0);
