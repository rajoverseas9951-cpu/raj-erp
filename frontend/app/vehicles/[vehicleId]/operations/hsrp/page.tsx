...
export default function HsrpPage() {
  ...
  const dealerLedgers = useMemo(
    () => ledgers.filter((l) => l.status === 'active' && l.ledger_group === 'Sundry Creditors'),
    [ledgers],
  );
  ...
  async function createDealerLedger() {
    ...
    try {
      const created = await ledgerApi.create({
        ledger_name: name.trim().toUpperCase(),
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await vehicleOperationsApi.create(vehicleId, 'hsrp', {
        ...
        amount: dealerCost,
        notes: dealerCost > 0
          ? `HSRP outsourced to ${selectedDealer?.ledger_name} | Dealer payable ${money(dealerCost)} | Monthly settlement`
          : 'HSRP customer charge only',
      });
      createdId = created.id;

      if (customerCharge > 0) {
        await vehicleOperationsApi.create(vehicleId, 'payment', {
          payment_type: 'HSRP Bill',
          account: 'HSRP SERVICE',
          issue_date: fd.get('order_date'),
          billed_amount: customerCharge,
          reference_number: `HSRP-${String(fd.get('order_date') || '')}`,
          notes: `HSRP customer charge | HSRP record ${created.id}`,
        });
      }

      if (dealerCost > 0 && selectedDealer) {
        await vehicleOperationsApi.create(vehicleId, 'agent_payment', {
          party_name: selectedDealer.ledger_name,
          account: 'HSRP DEALER PAYABLE',
          issue_date: fd.get('order_date'),
          billed_amount: dealerCost,
          paid_amount: 0,
          reference_number: `HSRP-${String(fd.get('order_date') || '')}`,
          notes: `HSRP dealer cost accrued for monthly settlement | HSRP record ${created.id}`,
        });
      }
      ...
      if (createdId) {
        try { await vehicleOperationsApi.remove(vehicleId, 'hsrp', createdId); } catch {}
      }
  ...
  return <main className="min-h-screen bg-[#f4f7fc] p-3 text-[#081a3a] sm:p-5 md:p-7">
    <div className="mx-auto max-w-7xl space-y-4">
      <section className="relative overflow-hidden rounded-[28px] border border-[#173d78] bg-[#071a3c] p-6 text-white shadow-[0_24px_70px_rgba(7,26,60,.20)]">
        ...
        <div className="relative flex items-center justify-between"><div><a href={`/vehicles/${vehicleId}`} className="text-xs font-bold text-blue-200">← Vehicle Profile</a><p className="mt-5 text-[9px] font-black uppercase tracking-[.24em] text-[#63d4ff]">High security registration plate</p><h1 className="mt-1 text-3xl font-black">HSRP</h1><p className="mt-2 text-xs text-blue-100/70">Customer billing + dealer payable, with monthly settlement.</p></div><div className="grid h-20 w-20 place-items-center rounded-[24px] border border-white/10 bg-white/10 text-4xl">▣</div></div>
      </section>
      ...
      <form onSubmit={submit} className="overflow-hidden rounded-[26px] border border-[#d9e5f7] bg-white shadow-[0_14px_40px_rgba(26,64,120,.08)]">
        ...
        <div className="mx-5 mb-5 grid gap-3 rounded-2xl border border-slate-200 bg-[#f8fbff] p-4 sm:mx-6 sm:grid-cols-3">
          <MoneyCard label="Customer Receivable" value={customerCharge} tone="blue" />
          <MoneyCard label="Dealer Payable" value={dealerCost} tone="amber" />
          <MoneyCard label="HSRP Margin" value={margin} tone={margin < 0 ? 'red' : 'green'} />
        </div>
