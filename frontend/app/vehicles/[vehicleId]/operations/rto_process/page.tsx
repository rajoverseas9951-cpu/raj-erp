...
export default function RtoProcessPage() {
  ...
  const customerLiability = useMemo(
    () => governmentPaidBy === 'owner' ? serviceOtherCharge : total,
    [governmentPaidBy, serviceOtherCharge, total],
  );
        ...
        <section className="relative overflow-hidden rounded-[30px] border border-[#173d78] bg-[#071a3c] p-5 text-white shadow-[0_24px_70px_rgba(7,26,60,.20)] sm:p-7">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-blue-100/70">Add the work once. Billing and statutory fee accounting are handled automatically.</p>
            </div>
        ...
        <form onSubmit={submit} className="overflow-hidden rounded-[28px] border border-[#d9e5f7] bg-white shadow-[0_16px_45px_rgba(26,64,120,.08)]">
          ...
          <section className="mx-5 mb-5 rounded-[24px] border border-slate-200 bg-[#fbfdff] p-5 sm:mx-6 sm:mb-6">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[9px] font-black uppercase tracking-[.18em] text-blue-600">Billing</p>
                <h3 className="mt-1 text-lg font-black text-slate-950">Customer Charge</h3>
              </div>
              <div className="rounded-xl bg-[#081f49] px-4 py-2.5 text-right text-white">
                <p className="text-[8px] font-black uppercase tracking-wider text-blue-200">Receivable</p>
                <p className="text-xl font-black">{money(customerLiability)}</p>
              </div>
            </div>

            <div className="grid gap-4 lg:grid-cols-3">
              <label className={labelClass}>Total Amount *
                <input name="amount" type="number" min="0" step="0.01" required value={totalAmount} onChange={(e) => setTotalAmount(e.target.value)} className={inputClass} />
              </label>

              <label className={labelClass}>Government Fee
                <input name="government_fee" type="number" min="0" step="0.01" value={governmentFee} onChange={(e) => { setGovernmentFee(e.target.value); if (Number(e.target.value || 0) <= 0) setGovernmentPaidBy('owner'); }} className={`${inputClass} ${splitInvalid ? 'border-rose-400 ring-4 ring-rose-50' : ''}`} />
                {splitInvalid && <span className="normal-case font-semibold tracking-normal text-rose-600">Cannot exceed Total Amount.</span>}
              </label>

              <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 px-4 py-3">
                <p className="text-[9px] font-black uppercase tracking-wider text-emerald-700">Service + Other</p>
                <p className="mt-1 text-2xl font-black text-slate-950">{money(serviceOtherCharge)}</p>
                <p className="mt-1 text-[10px] font-semibold text-slate-500">Auto: Total − Govt. Fee</p>
              </div>
            </div>

            {govt > 0 && (
              <div className="mt-4 grid gap-4 border-t border-slate-200 pt-4 lg:grid-cols-3">
                <label className={labelClass}>Government Fee Paid By
                  <select value={governmentPaidBy} onChange={(e) => setGovernmentPaidBy(e.target.value as GovtPayer)} className={inputClass}>
                    <option value="owner">Customer / Owner</option>
                    <option value="us">Office / Us</option>
                    <option value="agent">RTO Agent</option>
                  </select>
                </label>

                {governmentPaidBy === 'us' && (
                  <label className={labelClass}>Paid From Bank / Cash *
                    <select name="government_fee_bank_ledger_id" required className={inputClass}>
                      <option value="">Select Bank / Cash</option>
                      {bankLedgers.map((l) => <option key={l.id} value={l.id}>{l.ledger_name}</option>)}
                    </select>
                  </label>
                )}

                <div className="flex items-end">
                  <div className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-600">
                    {governmentPaidBy === 'owner' && <>Owner paid Govt. Fee directly. Only <b>{money(serviceOtherCharge)}</b> is due to us.</>}
                    {governmentPaidBy === 'us' && <>Govt. Fee will post against the selected Bank/Cash through the clearing ledger.</>}
                    {governmentPaidBy === 'agent' && <>Govt. Fee becomes payable to the RTO Agent. Customer owes the full <b>{money(total)}</b>.</>}
                  </div>
                </div>
              </div>
            )}
            ...
            <p className="mt-4 text-[10px] font-semibold text-slate-400">Government fee stays separate from RTO income and is reconciled through Government Fee Clearing.</p>
          </section>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-100 bg-[#f8fbff] p-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <p className="text-[10px] font-semibold text-slate-500">RTO income = Total Amount − Government Fee.</p>
            <button disabled={saving || splitInvalid} className="min-w-[190px] rounded-2xl bg-gradient-to-r from-[#0b2b62] to-[#2563eb] px-6 py-3.5 text-sm font-black text-white shadow-[0_12px_28px_rgba(37,99,235,.28)] transition hover:-translate-y-0.5 disabled:opacity-50">{saving ? 'Saving…' : '✓ Save RTO Work'}</button>
          </div>
        </form>
