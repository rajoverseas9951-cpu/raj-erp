...
export default function RtoProcessPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      element.reset(); setSelectedWorkType(renewalMode ? 'Renewal Registration' : ''); setFaceless(true); setRtoAgentEnabled(false); setTotalAmount(''); setGovernmentFee(''); setGovernmentPaidBy('owner'); await load();
  ...
  return <main className="min-h-screen bg-[#f4f7fc] p-3 text-[#081a3a] sm:p-5 lg:p-7"><div className="mx-auto max-w-[1450px] space-y-5">
    <section className="relative overflow-hidden rounded-[30px] border border-[#173d78] bg-[#071a3c] p-5 text-white shadow-[0_24px_70px_rgba(7,26,60,.20)] sm:p-7"><div className="absolute inset-0 bg-[radial-gradient(circle_at_88%_12%,rgba(43,117,255,.48),transparent_34%),linear-gradient(135deg,#06152f,#0a2555_60%,#0c3478)]"/><div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><a href={`/vehicles/${vehicleId}`} className="text-xs font-bold text-blue-200 hover:text-white">← Vehicle Profile</a><p className="mt-5 text-[9px] font-black uppercase tracking-[.24em] text-cyan-300">{renewalMode ? 'Registration renewal desk' : 'RTO work desk'}</p><h1 className="mt-1 text-3xl font-black tracking-tight sm:text-4xl">{renewalMode ? 'Renewal Registration' : 'RTO Process'}</h1><p className="mt-2 max-w-2xl text-sm text-blue-100/70">Enter total amount once. Government fee is the only split you select; service + other charge is automatic.</p></div><div className="rounded-2xl border border-white/10 bg-white/10 px-5 py-3 text-right backdrop-blur"><p className="text-[9px] font-black uppercase tracking-widest text-cyan-300">Total records</p><p className="mt-1 text-3xl font-black">{rows.length}</p></div></div></section>
    ...
    <form onSubmit={submit} className="overflow-hidden rounded-[28px] border border-[#d9e5f7] bg-white shadow-[0_16px_45px_rgba(26,64,120,.08)]">
      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 sm:p-6">
        <MasterSelect name="work_type" label="Work Type" required rows={workTypes} onAdd={() => void addWorkType()} value={selectedWorkType} onChange={setSelectedWorkType}/><Field name="receipt_date" label="Date" type="date" required/><Field name="reference_number" label="Application No" required/><MasterSelect name="rto_office" label="RTO Office" rows={rtoOffices.map((x) => ({ id: x.id, name: `${x.code ? `${x.code} · ` : ''}${x.name}` }))} onAdd={() => void addRtoOffice()} required/>
        <Toggle label="RTO Agent" enabled={rtoAgentEnabled} onChange={setRtoAgentEnabled}/>{rtoAgentEnabled && <Field name="external_agent" label="RTO Agent Name" required/>}{rtoAgentEnabled && <Field name="agent_amount" label="RTO Agent Amount" type="number" required/>}{vehicle?.broker_agent_enabled && <ReadOnlyField label="Broker" value={vehicle.broker_name || '—'}/>} {vehicle?.broker_agent_enabled && <ReadOnlyField label="Agent" value={vehicle.agent_name || '—'}/>} 
        <Toggle label="Faceless Appointment" enabled={faceless} onChange={setFaceless}/>{!faceless && <Field name="process_date" label="Application Date" type="date" required/>}<Field name="approval_date" label="Approve Date" type="date"/><Field name="rc_received_date" label="RC Rec Date" type="date"/><Field name="rc_delivered_date" label="RC Deliver Date" type="date"/><Field name="period" label="RC Status"/><label className={labelClass}>Status<select name="status" defaultValue="ACTIVE" className={inputClass}><option value="ACTIVE">Active</option><option value="PENDING">Pending</option><option value="COMPLETED">Completed</option><option value="CANCELLED">Cancelled</option></select></label>
      </div>
      ...
      <section className="mx-5 mb-5 overflow-hidden rounded-[26px] border border-blue-100 bg-gradient-to-br from-blue-50/80 via-white to-emerald-50/50 sm:mx-6 sm:mb-6">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-blue-100 px-5 py-4"><div><p className="text-[9px] font-black uppercase tracking-[.22em] text-blue-600">Billing Split</p><h3 className="mt-1 text-lg font-black text-slate-950">Total charge → automatic split</h3><p className="mt-1 max-w-2xl text-xs font-semibold text-slate-500">Enter Total Amount and Government Fee only. The system calculates your service + other charge automatically.</p></div><div className="rounded-2xl bg-[#081f49] px-5 py-3 text-right text-white shadow-lg"><p className="text-[9px] font-black uppercase tracking-wider text-blue-200">Customer Receivable</p><p className="mt-1 text-2xl font-black">{money(customerLiability)}</p></div></div>
        ...
        {govt > 0 && <div className="border-t border-blue-100 bg-white/70 p-5"><div className="grid gap-4 lg:grid-cols-3">
          <label className={labelClass}>Government Fee Paid By<select name="government_fee_paid_by" value={governmentPaidBy} onChange={(e)=>setGovernmentPaidBy(e.target.value as GovtPayer)} className={inputClass}><option value="owner">Owner / Customer directly</option><option value="us">Paid by us</option><option value="agent">Paid by RTO Agent</option></select><span className="normal-case font-semibold tracking-normal text-slate-400">This selection applies only to government fee.</span></label>
          {governmentPaidBy === 'us' && <label className={labelClass}>Paid From Bank / Cash *<select name="government_fee_bank_ledger_id" required className={inputClass}><option value="">Select account</option>{bankLedgers.map((l)=><option key={l.id} value={l.id}>{l.ledger_name}</option>)}</select><span className="normal-case font-semibold tracking-normal text-slate-400">Bank/cash entry posts automatically.</span></label>}
          <div className={`rounded-2xl border p-4 text-xs font-semibold ${governmentPaidBy === 'owner' ? 'border-blue-200 bg-blue-50 text-blue-800' : governmentPaidBy === 'us' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>{governmentPaidBy === 'owner' ? <>Owner paid government fee directly. Customer owes only {money(serviceOtherCharge)}.</> : governmentPaidBy === 'us' ? <>Office paid government fee. Customer owes full {money(total)}; government fee is recoverable, not income.</> : <>RTO agent paid government fee. Customer owes full {money(total)} and government fee becomes payable to that agent.</>}</div>
        </div></div>}
      </section>
