 ...
 return <main className="min-h-screen bg-[#f4f7fb] p-3 text-slate-950 sm:p-5 md:p-7">
  <div className="mx-auto max-w-7xl space-y-4 sm:space-y-5">
   ...
   <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_55px_rgba(15,23,42,.08)]">
    ...
    <nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-3 py-2 sm:px-5"><Tab href="#overview" label="Overview" active/>{availableModules.has('insurance')&&<Tab href={operationHref(vehicle.id,'insurance')} label="Insurance"/>}{availableModules.has('puc')&&<Tab href={operationHref(vehicle.id,'puc')} label="PUC"/>}{availableModules.has('fitness')&&<Tab href={operationHref(vehicle.id,'fitness')} label="Fitness"/>}{availableModules.has('permit')&&<Tab href={operationHref(vehicle.id,'permit')} label="Permit"/>}{availableModules.has('rto_process')&&<Tab href={operationHref(vehicle.id,'rto_process')} label="RTO"/>}{availableModules.has('payment')&&<Tab href={operationHref(vehicle.id,'payment')} label="Payments"/>}</nav>
   </section>
   ...
   {quickModules.length>0&&<section className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div><p className="text-xs font-black uppercase tracking-[.16em] text-blue-600">Do it faster</p><h2 className="mt-1 text-xl font-black">Quick Actions</h2></div><div className="mt-5 grid grid-cols-3 gap-2 sm:grid-cols-6">{quickModules.map(module=><QuickAction key={module} label={moduleLabels[module]} href={operationHref(vehicle.id,module)} status={profile.modules[module]?.status}/>)}</div></section>}
