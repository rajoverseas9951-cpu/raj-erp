...
export default function ReportDetailPage(){
  ...
  function exportCsv(){
    if(!data.rows.length)return; const cols=columns; const esc=(v:unknown)=>`"${String(v??'').replaceAll('"','""')}"`;
    const csv=[cols.map(label).map(esc).join(','),...data.rows.map(r=>cols.map(c=>esc(r[c])).join(','))].join('\n');
    const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download=`${report}-report.csv`; a.click(); URL.revokeObjectURL(a.href);
  }
  const summary=Object.entries(data.summary||{});
  return <main className="min-h-screen bg-[#f4f7fb] p-4 md:p-6 text-slate-950">
    <div className="mx-auto max-w-[1600px] space-y-6">
      <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#07122f] via-[#0e2d70] to-[#2367e8] p-6 text-white shadow-2xl md:p-8">
        <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-black uppercase tracking-[.25em] text-cyan-200">Report Master</p><h1 className="mt-2 text-3xl font-black md:text-4xl">{titles[report]||'Report'}</h1><p className="mt-2 text-sm text-blue-100">Live ERP data · tenant wise · print & export ready.</p></div><div className="flex gap-2"><Link href="/reports" className="rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 text-sm font-bold">All Reports</Link><button onClick={()=>window.print()} className="rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-blue-800">Print</button><button onClick={exportCsv} className="rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-black text-slate-950">Excel / CSV</button></div></div>
      </section>
      ...
      <section className="rounded-[28px] border bg-white p-4 shadow-sm md:p-5"><div className="grid gap-3 md:grid-cols-4"><label className="text-xs font-black uppercase text-slate-500">From<input type="date" value={from} onChange={e=>setFrom(e.target.value)} className="mt-2 w-full rounded-xl border px-3 py-3 text-sm"/></label><label className="text-xs font-black uppercase text-slate-500">To<input type="date" value={to} onChange={e=>setTo(e.target.value)} className="mt-2 w-full rounded-xl border px-3 py-3 text-sm"/></label><label className="text-xs font-black uppercase text-slate-500 md:col-span-1">Search<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Vehicle / policy / customer..." className="mt-2 w-full rounded-xl border px-3 py-3 text-sm"/></label><div className="flex items-end"><button onClick={()=>void load()} className="w-full rounded-xl bg-blue-700 px-4 py-3 font-black text-white">Apply Filter</button></div></div></section>
      {error&&<div className="rounded-2xl border border-red-200 bg-red-50 p-4 font-semibold text-red-700">{error}</div>}
      <section className="overflow-hidden rounded-[28px] border bg-white shadow-sm"><div className="flex items-center justify-between border-b p-5"><div><h2 className="text-xl font-black">Detailed Report</h2><p className="text-sm text-slate-500">{loading?'Loading...':`${data.rows.length} records`}</p></div></div><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50"><tr>{columns.map(c=><th key={c} className="whitespace-nowrap px-4 py-3 text-[11px] font-black uppercase tracking-wider text-slate-500">{label(c)}</th>)}</tr></thead><tbody>{!loading&&data.rows.map((r,i)=><tr key={i} className="border-t hover:bg-blue-50/40">{columns.map(c=><td key={c} className="whitespace-nowrap px-4 py-3 font-medium">{display(c,r[c])}</td>)}</tr>)}{!loading&&!data.rows.length&&<tr><td colSpan={Math.max(columns.length,1)} className="p-12 text-center text-slate-400">No records found.</td></tr>}</tbody></table></div></section>
    </div>
  </main>;
}
