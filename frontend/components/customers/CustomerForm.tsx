...
function findName(lines: string[]) {
  const blocked =
    /government|india|aadhaar|uidai|dob|birth|male|female|address|vid|download|issue|year|father|mother|husband|enrol/i;
  const markerIndex = lines.findIndex((line) =>
    /dob|date of birth|yob|year of birth|\bmale\b|\bfemale\b/i.test(line),
  );
  const candidates = (
    markerIndex > 0
      ? lines.slice(Math.max(0, markerIndex - 4), markerIndex)
      : lines
  )
...
function parseAadhaarText(
  ...
  const pins = [...allText.matchAll(/\b[1-9][0-9]{5}\b/g)].map(
    (match) => match[0],
  );
...
export function CustomerForm({ customer }: { customer?: Partial<Customer> }) {
  ...
  async function readAadhaar() {
    ...
    try {
      ...
      const location = extracted.pincode
        ? await lookupPincode(extracted.pincode)
        : {};
      setValues((old) => ({ ...old, ...extracted, ...location }));
      setSuccess(
        "Details fill ho gayi. Name, DOB aur pincode save se pehle check kar lena.",
      );
    } catch (err) {
      console.error(err);
      setError(
        "Aadhaar clear read nahi hua. Manual Entry se details bhar sakte ho.",
      );
    } finally {
      setReading(false);
    }
    ...
    <form onSubmit={submit} className="space-y-6">
      <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="bg-gradient-to-r from-slate-950 via-blue-950 to-blue-800 px-6 py-6 text-white">
          <p className="text-xs font-semibold uppercase tracking-[.25em] text-blue-200">
            {BRAND.brandName}
          </p>
          ...
          <p className="mt-1 text-sm text-blue-100">
            Aadhaar auto-fill ya manual entry—dono available hain.
          </p>
        </div>
        <div className="grid gap-3 p-5 sm:grid-cols-2">
          <Mode
            active={entryMode === "aadhaar"}
            title="Aadhaar Auto Fill"
            text="Front/back upload karke free OCR se details bharo."
            onClick={() => setEntryMode("aadhaar")}
          />
          <Mode
            active={entryMode === "manual"}
            title="Manual Entry"
            text="Aadhaar ke bina saari details manually bharo."
      ...
      {entryMode === "aadhaar" && (
        <Card
          ...
          subtitle="Address se sirf City, District, State aur Pincode auto-fill hoga."
        >
          ...
          <button
            ...
            onClick={readAadhaar}
            disabled={reading}
            className="mt-4 rounded-xl bg-blue-700 px-5 py-3 font-semibold text-white disabled:opacity-60"
          >
      ...
      <Card
        ...
        subtitle="First name, last name aur mobile required hain."
      >
        <Grid>
          <Input
            label="First Name"
      ...
      <Card title="Identity Details" subtitle="Optional identity numbers.">
        <Grid>
          <Input
            ...
            onChange={(v) =>
              setField("aadhaar_number", v.replace(/\D/g, "").slice(0, 12))
            }
          />
          <Input
            label="PAN Number"
            value={values.pan_number}
            onChange={(v) => setField("pan_number", v.toUpperCase())}
          />
          <Input
            label="Driving Licence"
            value={values.driving_licence_number}
            onChange={(v) =>
              setField("driving_licence_number", v.toUpperCase())
            }
          />
          <Input
            label="Passport Number"
            value={values.passport_number}
            onChange={(v) => setField("passport_number", v.toUpperCase())}
          />
          <Input
            label="Voter ID"
            value={values.voter_id}
            onChange={(v) => setField("voter_id", v.toUpperCase())}
          />
          <Input
            label="GST Number"
            value={values.gst_number}
            onChange={(v) => setField("gst_number", v.toUpperCase())}
          />
        </Grid>
      </Card>
      <Card
        title="Address"
        subtitle="Full address manual rahega; location pincode se auto-fill hogi."
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Textarea
...
function Mode({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-xl border p-4 text-left ${active ? "border-blue-600 bg-blue-50 ring-2 ring-blue-100" : "border-slate-200"}`}
    >
      <b>{title}</b>
  ...
  return (
    <label className="text-sm font-semibold text-slate-700">
      ...
      <input
        ...
        className="mt-2 w-full rounded-xl border px-4 py-3 font-normal outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-100"
      />
    </label>
  );
}
...
function Upload({
  ...
  return (
    <label className="block cursor-pointer rounded-2xl border-2 border-dashed bg-slate-50 p-4">
      <b>{label}</b>
      <span className="block text-sm text-slate-500">
        Clear image select karein
      </span>
      <input
        type="file"
      ...
      {preview ? (
        <img
          src={preview}
          alt={label}
          className="mt-4 h-44 w-full rounded-xl bg-white object-contain"
        />
      ) : (
        <div className="mt-4 flex h-32 items-center justify-center rounded-xl bg-white text-slate-400">
          No image selected
        </div>
      )}
    </label>
